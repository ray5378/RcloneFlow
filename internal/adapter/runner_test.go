...
func TestTaskRunnerImpl_Sync(t *testing.T) {
	...
	jobID, err := runner.RunTask(context.Background(), 2, "sync", "local", "/src", "gdrive", "/dst", "schedule", nil)
	if err != nil {
		t.Fatalf("RunTask() error = %v", err)
	}
	if jobID != 789 {
...
func TestTaskRunnerImpl_Move(t *testing.T) {
	...
	jobID, err := runner.RunTask(context.Background(), 3, "move", "local", "/src", "gdrive", "/dst", "manual", nil)
	if err != nil {
		t.Fatalf("RunTask() error = %v", err)
	}
	if jobID != 321 {
...
func TestTaskRunnerImpl_PathHandling(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := NewRcloneClient(&RcloneConfig{BaseURL: server.URL})
			runner := NewTaskRunner(client)
			if _, err := runner.RunTask(context.Background(), 1, "copy", "local", tt.srcPath, "gdrive", tt.dstPath, "manual", nil); err != nil {
				t.Fatalf("RunTask() error = %v", err)
			}
		})
	}
