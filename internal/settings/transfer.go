...
func Load() (TransferSettings, error) {
	...
	// 兜底默认
	def := defaults()
	if ts.PostVerifyMode == "" {
		ts.PostVerifyMode = def.PostVerifyMode
	}
	if ts.PostVerifyInterval == "" {
		ts.PostVerifyInterval = def.PostVerifyInterval
	}
	if ts.PostVerifyTimeout == "" {
		ts.PostVerifyTimeout = def.PostVerifyTimeout
	}
	if ts.PostVerifyMtimeGrace == "" {
		ts.PostVerifyMtimeGrace = def.PostVerifyMtimeGrace
	}
	if ts.PostVerifyMatch == "" {
		ts.PostVerifyMatch = def.PostVerifyMatch
	}
	if ts.MinRerunInterval == "" {
		ts.MinRerunInterval = def.MinRerunInterval
	}
	return ts, nil
}
