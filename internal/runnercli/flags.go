...
func buildFlagsFromOptions(opt map[string]any) []string {
	...
	// 常用数值
	if v, ok := asInt(opt["transfers"]); ok {
		push("--transfers", v)
	}
	if v, ok := asInt(opt["checkers"]); ok {
		push("--checkers", v)
	}
	if v, ok := asInt(opt["retries"]); ok {
		push("--retries", v)
	}
	if v, ok := asInt(opt["lowLevelRetries"]); ok {
		push("--low-level-retries", v)
	}
	...
	for _, key := range []string{"ignoreExisting", "checksum", "sizeOnly", "ignoreSize", "ignoreTimes", "update", "noTraverse", "noCheckDest", "inplace", "immutable", "checkFirst", "deleteBefore", "deleteDuring", "deleteAfter", "trackRenames", "ignoreErrors", "useServerModtime", "refreshTimes", "deleteExcluded", "dryRun", "serverSideAcrossConfigs", "interactive"} {
		if b, ok := asBool(opt[key]); ok && b {
			push("--" + toKebab(key))
		}
	}
	// 特殊布尔：disableHttp2 → --disable-http2
	if b, ok := asBool(opt["disableHttp2"]); ok && b {
		push("--disable-http2")
	}
	// 路径类
	if s, ok := asStr(opt["compareDest"]); ok {
		push("--compare-dest", s)
	}
	if s, ok := asStr(opt["copyDest"]); ok {
		push("--copy-dest", s)
	}
	if s, ok := asStr(opt["backupDir"]); ok {
		push("--backup-dir", s)
	}
	...
	// include
	{
		...
		if arr, ok := asArr(opt["include"]); ok {
			for _, p := range arr {
				p = norm(p)
				if p != "" {
					incSet[p] = struct{}{}
	...
	// exclude
	{
		...
		if arr, ok := asArr(opt["exclude"]); ok {
			for _, p := range arr {
				p = norm(p)
				if p != "" {
					excSet[p] = struct{}{}
	...
	// 超时：仅在显式配置时映射
	if s, ok := asInt(opt["timeout"]); ok {
		push("--timeout", s+"s")
	}
	if s, ok := asInt(opt["connTimeout"]); ok {
		push("--contimeout", s+"s")
	}
	if s, ok := asInt(opt["expectContinueTimeout"]); ok {
		push("--expect-continue-timeout", s+"s")
	}
	// 其他
	if s, ok := asStr(opt["bwLimit"]); ok {
		push("--bwlimit", s)
	}
	if s, ok := asInt(opt["maxTransfer"]); ok {
		push("--max-transfer", s)
	}
	if s, ok := asInt(opt["maxDuration"]); ok {
		push("--max-duration", s+"s")
	}
	if s, ok := asStr(opt["logFile"]); ok {
		push("--log-file", s)
	}
	return flags
}
