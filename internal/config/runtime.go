...
func GetFinishWaitInterval() time.Duration {
	...
	if v := readOverrides()["FINISH_WAIT_INTERVAL"]; v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			return d
		}
	}
...
func GetFinishWaitTimeout() time.Duration {
	if v := os.Getenv("FINISH_WAIT_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			return d
		}
	}
	if v := readOverrides()["FINISH_WAIT_TIMEOUT"]; v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			return d
		}
	}
...
// GetWebhookMaxFiles returns max file names to include in webhook payload (settings.json overrides)
// 0 means unlimited
func GetWebhookMaxFiles() int {
	if v := readOverrides()["WEBHOOK_MAX_FILES"]; v != "" {
