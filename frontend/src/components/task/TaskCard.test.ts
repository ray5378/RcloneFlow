...
describe('TaskCard', () => {
  it('keeps task cards bordered, path details unboxed, and hover elevated without recoloring', () => {
    ...
    expect(baseStyles).toContain(':global(body.light .task-card:hover)')
    expect(baseStyles).toContain(':global(body.light .run-item:hover)')
    expect(baseStyles).toContain('background: #f8f8f8')
