...
const displayRuns = computed(() => props.filteredRuns.map(run => {
  ...
  return {
    ...
    __statusText: run.status === 'finished' ? t('runItem.finished') : run.status === 'failed' ? t('runItem.failed') : run.status === 'skipped' ? t('runItem.skipped') : run.status === 'running' ? t('runItem.running') : run.status === 'stopped' ? t('runItem.stopped') : run.status,
    __statusClass: run.status === 'finished' ? 'success' : run.status === 'failed' ? 'danger' : run.status === 'skipped' ? 'warning' : run.status === 'running' ? 'info' : run.status === 'stopped' ? 'info' : '',
