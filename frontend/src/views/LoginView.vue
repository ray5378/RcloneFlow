<template>
  <div class="login-page">
    <div class="login-card">
      ...
      <form @submit.prevent="isSetup ? handleSetupSubmit() : handleLoginSubmit()">
        ...
        <div v-if="isSetup" class="field-item">
        ...
        <button type="submit" class="primary-btn" :disabled="loading">
          <template v-if="loading">{{ isSetup ? t('login.creating') : t('login.submitting') }}</template>
          <template v-else>{{ isSetup ? t('login.createAccount') : t('login.submit') }}</template>
        </button>
      </form>
    </div>
  </div>
...
import { login, register, setTokens } from '../api/auth'
import { t } from '../i18n'

const props = defineProps<{ hasExistingUsers: boolean }>()
...
const isSetup = computed(() => !props.hasExistingUsers)
