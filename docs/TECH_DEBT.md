...
#### 4.1 `TaskView.vue` 收尾优化与防反弹（阶段性收口）
现状：
- 页面主链已基本收敛为装配层
- 本轮已完成分段标记、顺序 guardrail 文档化，以及 shell 无用透传的小步清理
- 当前主要目标不再是继续重构，而是防止后续维护把它重新改回高风险大装配文件
...
后续策略：
- 默认视为已阶段性收口
- 若再推进，只允许做极小的、防反弹性质的整理，不再把它作为当前主技术债主线

#### 4.2 固化 `progress / completedFreezeByTask / finalSummary` 契约边界（本轮已完成两轮加固）
现状：
- 已完成前端类型、关键 composable guardrail、接口文档同步
- 已完成后端注释与自动化测试下沉，明确禁止 `finalSummary` 回流到 `/api/runs/active`
- 这条线当前已经从“靠文档记忆”推进到“文档 + 类型 + 注释 + 测试”共同约束
...
#### 4.6 `web/` 已切换为可重建构建输出目录
现状：
- `web/` 由前端构建生成，不再作为受管源码手工维护
- Dockerfile 已具备独立前端构建能力，本地也可通过 `cd frontend && npm run build` 生成

当前规则：
- 前端源码只维护在 `frontend/`
- `web/index.html` 与 `web/assets/*` 属于构建输出，不再要求手工提交
- 不要把构建输出误当成业务源码改动

后续重点：
- 让开发流程、发布流程、提交检查都围绕“源码可重建产物”执行
- 本地直跑若缺少 `web/`，应先构建前端或切换到 Docker 运行
...
当前规则：
- 本项目里的 SQLite 优先追求稳定性，不追求数据库并发吞吐
- 后续若新增 SQLite 打开点，应默认沿用单连接策略，避免重新引入同进程多连接写竞争
- 若后续仍出现 `database is locked`，优先排查跨进程竞争、长事务占锁或绕过主链的独立打开点
